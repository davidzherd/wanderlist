...
export function AddLocationView() {
  ...
  return (
    <div className="h-full w-full overflow-y-auto bg-sand-light px-4 py-8 dark:bg-espresso sm:px-6">
      <div className="mx-auto max-w-xl">
        <h1 className="mb-1 font-display text-2xl font-semibold text-espresso dark:text-sand-light">
          Add a bucket list location
        </h1>
        <p className="mb-6 text-sm text-espresso/60 dark:text-sand-light/60">
        ...
        <div className="glass-panel mb-6 rounded-2xl p-4">
          <label className="relative block">
            <Search size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-espresso/40 dark:text-sand-light/40" />
            <input
              type="text"
              value={query}
            ...
            />
            {isSearching && (
              <Loader2 size={16} className="absolute right-3 top-1/2 -translate-y-1/2 animate-spin text-espresso/40 dark:text-sand-light/40" />
            )}
          </label>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="glass-panel flex flex-col gap-4 rounded-2xl p-6">
          ...
          <button
            ...
            className="mt-2 flex items-center justify-center gap-2 self-start rounded-full bg-terracotta px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
          >
